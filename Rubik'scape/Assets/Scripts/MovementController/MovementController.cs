...
public class MovementController : MonoBehaviour
{
    ...
    void Update() {
        if (target == null) { // Si on n'a pas de cible, chercher les empty objects à la même distance
            FindNearestEmptyObjects();
        }
        Debug.Log("En attente d'entrée utilisateur ? " + isWaitingForInput); // DEBUG

        if (isWaitingForInput) { // Si on attend l'entrée de l'utilisateur, vérifier si une touche est pressée
            HandleUserInput();
        }

        if (target != null && !isWaitingForInput) { // Si une cible est définie, se déplacer vers elle
            MoveTowardsTarget();
        }
    }
